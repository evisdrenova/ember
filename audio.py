...
def simple_tts_say(text: str) -> bool:
    """Simple TTS - just say the text without any streaming complexity."""
    try:
        from kittentts import KittenTTS
        import numpy as np
        import subprocess
        ...
        # Simple config
        model_id = os.getenv("KITTEN_MODEL", "KittenML/kitten-tts-nano-0.1")
        voice = os.getenv("KITTEN_VOICE", "expr-voice-3-f")
        ...
        # Generate audio (returns float32 array)
        ...
        if not isinstance(audio, np.ndarray):
            audio = np.array(audio, dtype=np.float32)
        audio = np.clip(audio, -1.0, 1.0)
        pcm_data = (audio * 32767.0).astype(np.int16).tobytes()
        
        # Play with aplay
        aplay = subprocess.Popen([
            "aplay", "-q", "-D", output_device, 
            "-r", "24000", "-f", "S16_LE", "-t", "raw"
        ], stdin=subprocess.PIPE)
